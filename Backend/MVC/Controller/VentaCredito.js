...
const NuevaVentaCredito = async (req, res) => {
    try {
        const { IDVenta, FechaPedido, Subtotal, Total, Credito, IDCliente, IDEmpleado } = req.body;
        const sql = 'INSERT INTO Compra (  IDVenta, FechaPedido, Subtotal, Total, Credito, IDCliente, IDEmpleado) VALUES (?,?, ?, ?, ?, ?, ?)';
        const result = await pool.query(sql, [parseInt(IDVenta), FechaPedido, parseFloat(Subtotal), parseFloat(Total), parseInt(Credito), parseInt(IDCliente), parseInt(IDEmpleado)]);
...
const HistorialCreditos = async (req, res) => {
    try {
        const [rows, fields] = await pool.query('SELECT IDVenta, FechaPedido, Subtotal, Total, Credito, IDCLiente, IDEmpleado FROM Venta WHERE Credito = 1');
